...
class TrainConfig():    
    r""" Class to store the training configuration of the model.

    Args:
        name (str): Name the model
        folder (str, optional)
        checkpoint (Union[None, str], optional): 
            Path to a previous checkpoint to load from, if none no checkpoint is loaded, default None
        tensor_board (Union[None, str], optional): 
            Path to save tensorboard of training progress, if none then not saved.
...
class GNN(nn.Module):
    ...
    def __init__(self,
        ...
        """Initialise base class for all the models.
    ...
    def load_model(self, arch, weights, checkpoint):
        """
        Loads model architecture, weights and checkpoint info from the desired source if available, and loads to correct device.
        """
        ...
        # If checkpoint file is provided 
